...
# This step, 'delete-existing-ec2-resources,' uses the Amazon AWS CLI in a Docker container to check if a CloudFormation stack named 'devops-stack' exists in the 'us-east-1' region. If it does, it deletes the stack and waits for the deletion to complete.
...
# This step, 'create-ec2-resources-after-deletion,' uses an Amazon AWS CLI Docker container. It configures AWS credentials, then creates EC2 resources by launching a CloudFormation stack named 'devops-stack' using a template file ('cf.yml') in the 'us-east-1' region, and waits for the stack creation to complete. the environment variable are set in circle ci.
...
aws configure set default.region us-east-1 - run:
...
# This 'create-eks-cluster-app' step, employing an Amazon AWS CLI Docker container, creates a Kubernetes cluster called 'devopsproject' in the 'us-east-1' region. It verifies the node status, establishes a Kubernetes namespace named 'node-namespace,' updates the 'kubeconfig' for 'devopsproject,' and deploys version 1 of an application ('deploy.yml'). Finally, it references the 'destroy-cluster' step.
...
aws eks update-kubeconfig --name devopsproject --region us-east-1 - run:
name: Deploying v1
command: |
kubectl apply -f deploy.yml - destroy-cluster
